...
text \<open>
  Higher-order derivatives are expressed directly via the canonical library iterate
  \<open>(deriv ^^ n) f\<close> (theory Real\_and\_Complex\_Analytic.Limits\_Higher\_Order\_Derivatives),
  not a locally re-defined recursive function and not a convenience abbreviation either --
  every occurrence below is written out as \<open>(deriv ^^ n) f\<close>.
\<close>
...
next
  ...
  proof -
    ...
    have "(deriv ^^ (Suc n)) sigmoid x = deriv (\<lambda> w. (deriv ^^ n) sigmoid w) x"
      by simp    
...
proof -
  ...
  proof - 
    have differentiable_terms: "\<And>k. 1 \<le> k \<and> k \<le> n+1 \<Longrightarrow> 
      ...
      by(clarify, simp add: field_differentiable_imp_differentiable sigmoid_differentiable')
    then show ?thesis
      by(subst differentiable_sum, simp_all)
  qed
  then show ?thesis
...
proof -
  ...
  show ?thesis
    by (subst val_eq[symmetric], rule deriv_sigmoid_has_deriv)
qed

...
text \<open>
  In this theory, we formalize the Universal Approximation Theorem (UAT) for continuous functions
  defined on a closed interval [a, b]. The theorem states that any continuous function can
  be uniformly approximated by a finite sum of shifted and scaled sigmoidal functions. 
  This work is based on the article "Constructive Approximation by Superposition of Sigmoidal Functions" by 
  by Danilo Costarelli and Renato Spigler.  
\<close>

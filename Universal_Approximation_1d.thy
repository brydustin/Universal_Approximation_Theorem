...
definition G_network :: "(real \<Rightarrow> real) \<Rightarrow> (real \<Rightarrow> real) \<Rightarrow> real \<Rightarrow> real \<Rightarrow> nat \<Rightarrow> real \<Rightarrow> real \<Rightarrow> real" where
  "G_network \<sigma> f a b N w x =
    ...
    + f a * \<sigma> (w * (x - unif_part a b N ! 0))"

(* Theorem 2.1, proof estimate: any sufficiently fine N and saturated weight work. *)
lemma sigmoidal_approximation_fixed_parameters:
  fixes \<sigma> f :: "real \<Rightarrow> real" and N :: nat and w \<eta> \<delta> \<epsilon> a b :: real
  assumes bounded_sigmoidal: "bounded_function \<sigma>"
    and a_lt_b: "a < b" and contin_f: "continuous_on {a..b} f"
    and eta_eq: "\<eta> = \<epsilon> / (Sup ((\<lambda>x. \<bar>f x\<bar>) ` {a..b}) + 2 * Sup ((\<lambda>x. \<bar>\<sigma> x\<bar>) ` UNIV) + 2)"
    and \<eta>_pos: "0 < \<eta>" and \<delta>_pos: "0 < \<delta>"
    and \<delta>_prop: "\<forall>x\<in>{a..b}. \<forall>y\<in>{a..b}. \<bar>x-y\<bar> < \<delta> \<longrightarrow> \<bar>f x-f y\<bar> < \<eta>"
    and N_defining_properties: "real N > 2*(b-a)/\<delta> \<and> N > 3 \<and> real N > 1/\<eta>"
    and sat: "\<forall>k < N+2.
       (\<forall>x. x - unif_part a b N ! k \<ge> (b-a)/N \<longrightarrow> \<bar>\<sigma> (w*(x-unif_part a b N ! k))-1\<bar> < 1/N) \<and>
       (\<forall>x. x - unif_part a b N ! k \<le> -((b-a)/N) \<longrightarrow> \<bar>\<sigma> (w*(x-unif_part a b N ! k))\<bar> < 1/N)"
  defines "xs \<equiv> unif_part a b"
  shows "\<forall>x\<in>{a..b}. \<bar>(\<Sum>k = 2..N + 1. (f (xs N ! k) - f (xs N ! (k - 1))) * \<sigma> (w * (x - xs N ! k))) + f a * \<sigma> (w * (x - xs N ! 0)) - f x\<bar> < \<epsilon>"
proof -
  ...
  have w_prop: "\<forall>k < length (xs N).
    ...
    using sat unfolding h_def xs_def by (simp only: length_unif_part minus_divide_left)
  (* Define G_Nf (Equation 2.2) *)  
  (* G_Nf is the “network output”: a finite sum of \<sigma>-shifts with data-driven coefficients
     (forward differences of f at grid points) plus a bias term f(a)\<sigma>(w(x - x_0)). *)
  ...
  (* We will localize around the unique subinterval containing x and decompose the error
     into I1 (network vs. local “two-step” proxy L_i) and I2 (proxy vs. f). *)

  show "\<forall>x\<in>{a..b}. \<bar>(\<Sum>k = 2..N + 1. (f (xs N ! k) - f (xs N ! (k - 1))) * \<sigma> (w * (x - xs N ! k))) + f a * \<sigma> (w * (x - xs N ! 0)) - f x\<bar> < \<epsilon>"
  proof (intro ballI)
    fix  x::real
    ...
    (* L_i is a local two-sigmoid proxy that keeps only the “active” steps around i,i+1
       and replaces the rest by telescoping sums (a constant on the segment). *)
    ...
    (* I1 measures the cumulative \<sigma>-step-approximation error away from the two local steps. *)
    ...
    (* I2 captures the local interpolation error of f within the i-th cell. *)
    obtain I_2 where I_2_def: "\<And>i. 1 \<le> i \<and> i \<le> N \<longrightarrow> I_2 i = (\<lambda>x. \<bar>L i x - f x\<bar>)"
      by force

    (* Triangle inequality to split the total error. *)
...
i.e.  Show that x -   xs N!k   \<ge> h for k \<in> { 0,...,i-1} *)
    (* This ensures the “left-of-x” sigmoids are saturated near 1 with error < 1/N. *)
    ...
    (* This ensures the “right-of-x” sigmoids are saturated near 0 with error < 1/N. *)
    ...
    (* Bound I1: contributions far from x are small because the sigmoids are saturated
       with error < 1/N and \<Sum> of differences telescopes or is small termwise. *)
    ...
    "I_1 i x \<le> (\<Sum>k\<in>{2..i-1}. \<bar>f (xs N ! k) - f (xs N ! (k - 1))\<bar> * \<bar>\<sigma> (w * (x - xs N ! k)) - 1\<bar>)
      ...
      proof (cases "i < 3")
        (* Edge case: i=1 or i=2. Only the “future” steps contribute on the right, and we peel
           the first few terms explicitly; the bottom sum over {2..i-1} is empty. *)
        assume i_lt_3: "i < 3"
      ...
      next  
        (* Main case: i\<ge>3. Split the full sum into “below i” and “from i onward”
           and massage to isolate the two active steps at i and i+1. *)
      ...
      - f(a) -  (f (xs N ! (i)) - f (xs N ! (i - 1)))* \<sigma> (w * (x - xs N ! (i))) -
        ...
        show ?thesis
        proof - 
          (* The rest bounds both partial sums using |\<sigma> - 1| < 1/N or |\<sigma>| < 1/N
             and converts each f-difference factor to < \<eta> by uniform continuity. *)
        ...
        qed
      qed
      (* Now convert every factor to \<eta>*(1/N) using:
         - uniform continuity for |f(x_k)-f(x_{k-1})| < \<eta>,
         - step bounds |\<sigma>-1|<1/N on the left and |\<sigma>|<1/N on the right. *)
      ...
      proof(cases "i \<ge> 3")
          ...
          next
            ...
            proof(cases "\<forall>k. k \<in> {i+2..N+1} \<longrightarrow> \<bar>\<sigma> (w * (x - xs N ! k))\<bar> = 0")
              ...
              proof -
                ...
                qed
            next
              (* Non-degenerate: both lower and upper sums have active terms. *)
        ...
        next
          ...
          (* Mirror of the previous reasoning but for the lower block {2..i-1}. *)
        ...
        qed
      next
        (* Small i (\<le>2): the bottom sum is empty; only the top sum contributes. *)
        assume "\<not> 3 \<le> i"
      ...
      proof -       
        (* The two disjoint sums cover a subset of {2..N+1}. *)
      ...
      proof - 
        (* From the step lemma for k=0 (left of a): |\<sigma>(w(x-x_0)) - 1| < 1/N. *)
        have "\<bar>\<sigma> (w * (x - xs N ! 0)) - 1\<bar> < 1/N"
          by (smt (z3) Suc_eq_plus1_left w_prop add_2_eq_Suc' add_gr_0 atLeastAtMost_iff diff_zero 
              length_map length_upt x_in_ab xs_eqs zero_less_one zeroth_element)
        then show ?thesis
    ...
    (* Distance from x to the left node is < \<delta>; this will feed I2 via uniform continuity. *)
    ...
    qed
    (* Bound I2: use telescoping to rewrite the constant part and then apply uniform
       continuity + bounds on \<sigma> to show I2 \<le> (2 sup|\<sigma>| + 1)\<sqdot>\<eta>. *)
    ...
    next
      (* Small i (\<le>2): specialized algebra but same structure—two \<sigma> terms plus a base term. *)
      assume "\<not> 3 \<le> i"
      then have i_is_1_or_2: "i = 1 \<or> i = 2"
        using i_ge_1 by linarith
    ...
    (* Combine I1 and I2 bounds, fold \<eta> choice back to \<epsilon>. *)
...
(* Theorem 2.1: uniform approximation. *)
...
(* High-level plan:
   1) Pick \<eta> > 0 (in terms of \<epsilon> and bounds on f and \<sigma>) that will serve as a local tolerance.
   2) Use uniform continuity of f on [a,b] to get \<delta> > 0 such that |x-y|<\<delta> \<Rightarrow> |f x - f y|<\<eta>.
   3) Choose N large so that mesh size h=(b-a)/N is small relative to \<delta> and 1/N < \<eta>.
   4) Build a uniform partition xs N of [a,b] extended by one point on each side.
   5) Use the sigmoidal step-approximation lemma to pick w so that \<sigma>(w(\<sqdot>-x_k)) behaves
      like a sharp step across each node x_k with tolerance 1/N.
   6) Define the network G_Nf (finite linear combination of \<sigma>-shifts) and show
      |G_Nf(x)-f(x)| \<le> I1 + I2, where:
        - I1 controls the \<sigma>-step approximation error,
        - I2 controls the local interpolation error of f on the adjacent subinterval.
   7) Bound I1 by (1 + sup_{[a,b]}|f|)\<sqdot>\<eta> and I2 by (2 sup_{\<real>}|\<sigma>| + 1)\<sqdot>\<eta>.
   8) Choose \<eta> so that the sum is < \<epsilon>. *)
proof-
  ...
  (* Show \<eta> > 0: denominators are finite and positive, \<epsilon>>0 *)
  have \<eta>_pos: "\<eta> > 0"
    unfolding \<eta>_def
  proof -
  ...
  (* Choose N large enough to control both geometry (via h) and 1/N < \<eta>. *)
  obtain N where N_def: "N = (nat (\<lfloor>max 3 (max (2 * (b - a) / \<delta>) (1 / \<eta>))\<rfloor>) + 1)"
    by simp


  (* Core growth properties of N: ensures h small and 1/N < \<eta> later. *)
  ...
  (* Apply Lemma 2.1 to xs N and h to acquire \<omega>. *)
  ...
  (* This step provides a threshold \<omega> so that for any w \<ge> \<omega> each \<sigma>(w(\<sqdot>-x_k)) behaves like a step:
     it is within 1/N of 1 to the right by \<ge>h and within 1/N of 0 to the left by \<ge>h. *)
  ...
  have sat_N: "\<forall>k < N+2.
       (\<forall>x. x - unif_part a b N ! k \<ge> (b-a)/N \<longrightarrow> \<bar>\<sigma> (w*(x-unif_part a b N ! k))-1\<bar> < 1/N) \<and>
       (\<forall>x. x - unif_part a b N ! k \<le> -((b-a)/N) \<longrightarrow> \<bar>\<sigma> (w*(x-unif_part a b N ! k))\<bar> < 1/N)"
    using w_prop unfolding h_def xs_def by (simp only: length_unif_part minus_divide_left)
  have bound: "\<forall>x\<in>{a..b}. \<bar>(\<Sum>k = 2..N + 1. (f (xs N ! k) - f (xs N ! (k - 1))) * \<sigma> (w * (x - xs N ! k))) + f a * \<sigma> (w * (x - xs N ! 0)) - f x\<bar> < \<epsilon>"
    unfolding xs_def
    by (rule sigmoidal_approximation_fixed_parameters[OF bounded_sigmoidal a_lt_b contin_f
